...
OMX_PTR Exynos_OSAL_SharedMemory_Alloc(OMX_HANDLETYPE handle, OMX_U32 size, MEMORY_TYPE memoryType)
{
    ...
    switch (memoryType) {
    case SECURE_MEMORY:
        mask = ION_HEAP_EXYNOS_DRM_MFC_SECURE_MASK;
        break;
    case NORMAL_MEMORY:
...
#ifdef USE_DMA_BUF
    IONBuffer = ion_alloc((ion_client)pHandle->hIONHandle, size, 0, mask, 0);
#else
    IONBuffer = ion_alloc((ion_client)pHandle->hIONHandle, size, 0, mask);
#endif
    if (IONBuffer <= 0) {
