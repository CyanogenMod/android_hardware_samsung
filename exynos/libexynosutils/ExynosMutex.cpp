...
{
    if (m_flagCreate == true) {
        SLOGE("%s::Already created", __func__);
        return false;
    }
    ...
    case TYPE_SHARED:
        ...
        break;
    default:
        SLOGE("%s::unmatched type(%d) fail", __func__, type);
        return false;
        break;
    }
    ...
    if (m_mutex == NULL) {
        SLOGE("%s::Mutex create fail", __func__);
        return false;
    }
...
void ExynosMutex::destroy(void)
{
    if (m_flagCreate == false) {
        SLOGE("%s::Not yet created", __func__);
        return;
    }
...
bool ExynosMutex::lock(void)
{
    ...
    SLOGD("%s::%s'lock() start", __func__, m_name);
#endif

    if (((Mutex *)m_mutex)->lock() != 0) {
        SLOGE("%s::m_core->lock() fail", __func__);
        return false;
    }

#ifdef EXYNOS_MUTEX_DEBUG
    SLOGD("%s::%s'lock() end", __func__, m_name);
#endif
...
#ifdef EXYNOS_MUTEX_DEBUG
    SLOGD("%s::%s'unlock() start", __func__, m_name);
#endif

    ((Mutex *)m_mutex)->unlock();

#ifdef EXYNOS_MUTEX_DEBUG
    SLOGD("%s::%s'unlock() end", __func__, m_name);
#endif
...
#ifdef EXYNOS_MUTEX_DEBUG
    SLOGD("%s::%s'trylock() start", __func__, m_name);
#endif

    ret = ((Mutex *)m_mutex)->tryLock();

#ifdef EXYNOS_MUTEX_DEBUG
    SLOGD("%s::%s'trylock() end", __func__, m_name);
#endif
...
{
    if (handle == NULL) {
        SLOGE("%s::handle is null", __func__);
        return false;
    }
