                         ...
                         size_t bytes)
{
    ...
    out->pcm = pcm_open(CARD_DEFAULT, PORT_PLAYBACK, PCM_OUT | PCM_MMAP, &out->config);
...
static uint32_t in_get_sample_rate(const struct audio_stream *stream)
{
    ...
    LOGV("%s: requested_rate = %u\n", __func__, in->requested_rate);
...
static size_t in_get_buffer_size(const struct audio_stream *stream)
{
    ...
    LOGV("%s: period_size = %d\n", __func__, in->config.period_size);
    LOGV("%s: samplerate stream = %d\n", __func__, in_get_sample_rate(stream));
    LOGV("%s: rate = %d\n", __func__, in->config.rate); 
    size = (in->config.period_size * in_get_sample_rate(stream)) / in->config.rate;
    size = ((size + 15) / 16) * 16;

    ret = size * audio_stream_frame_size((struct audio_stream *)stream);
    LOGV("%s: buffersize = %u\n", __func__, ret);
                                   ...
                                   struct audio_stream_out **stream_out)
{
    ...
    *sample_rate = out_get_sample_rate(&out->stream.common);
    ...
    out->config.period_count = OUT_LONG_PERIOD_COUNT;
                                         ...
                                         int channel_count)
{
    ...
    */
    size = (IN_PERIOD_SIZE * sample_rate) / IN_SAMPLING_RATE;
    size = ((size + 15) / 16) * 16;

    ret = size * popcount(channel_count) *
